...
test("테마 토글 버튼 클릭 시 테마가 변경된다.", async ({ page, gnbPage }) => {
...
test.describe("책 검색 바 테스트", () => {
  test("키워드 검색 시 해당 페이지로 이동한다.", async ({ page, gnbPage }) => {
    await gnbPage.searchKeywordBySearchBar("하루키");
    await expect(page).toHaveURL("/search?query=하루키");
  });

  test("키워드 입력 후 해당 책 제목을 클릭하면 책 상세 페이지로 이동한다.", async ({ page, gnbPage }) => {
