    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-64 p-4" aria-label="월 선택 달력">
        <div className="mb-2 flex items-center justify-between">
          <Button variant="outline" size="sm" onClick={() => setCurrentYear(currentYear - 1)} aria-label="이전 년도">
            ←
          </Button>
          <span className="font-semibold">{currentYear}년</span>
          <Button variant="outline" size="sm" onClick={() => setCurrentYear(currentYear + 1)} aria-label="다음 년도">
            →
          </Button>
        </div>
        <div className="grid grid-cols-4 gap-2">
          {months.map((month, idx) => (
            <Button
              key={idx}
              variant={date.getMonth() === month.getMonth() && date.getFullYear() === month.getFullYear() ? 'default' : 'outline'}
              onClick={() => {
                setDate(month)
                setOpen(false)
              }}
