      ...
      <PopoverAnchor className={cn(className)}>
        <Command shouldFilter={false} className="rounded-lg border shadow-md">
          ...
          <PopoverContent className="w-[360px] p-0" align="start" onOpenAutoFocus={(e) => e.preventDefault()}>
            <CommandList>
              {query.length < 2 && <div className="py-6 text-center text-sm text-muted-foreground">검색어를 2자 이상 입력해주세요.</div>}

              {showLoading && <Loading text="검색 중..." variant="inline" />}

              {showResults && books.length === 0 && <CommandEmpty>검색된 책이 없습니다.</CommandEmpty>}
              ...
              {showResults && books.length > 0 && (
                <CommandGroup heading="검색 결과">
                  {books.map((book: SearchedBook) => (
                    ...
                    >
                      <BookThumbnail title={book.title} thumbnailUrl={book.thumbnailUrl} className="h-14 w-10" />
                      <div className="flex min-w-0 flex-1 flex-col">
                        <span className="truncate font-medium">{book.title}</span>
                        <span className="truncate text-xs text-muted-foreground">{book.author.name}</span>
                      </div>
                    </CommandItem>
                  ))}
                </CommandGroup>
