 ...
 */
export async function updatePost(id: number, input: UpdatePost): Promise<PostWithBook> {
  const userId = await requireUserId()
  if (!input.book) throw new Error('책 정보가 필요합니다')
...
async function paginatePosts(
  ...
  const cursorId = typeof cursor === 'string' && cursor !== '' ? Number(cursor) : typeof cursor === 'number' ? cursor : undefined
 ...
 */
export async function getPosts(take: number, cursor?: string): Promise<PaginationResponse<PostWithBook>> {
 ...
 */
export async function getPostsByUser(targetUserId: string, take: number, cursor?: string): Promise<PaginationResponse<PostWithBook>> {
  const me = await getCurrentUserId()
  const isOwn = me === targetUserId
  return paginatePosts(isOwn ? { userId: targetUserId } : { userId: targetUserId, isPrivate: false }, take, cursor)
}

/**
 * 본인 post 목록 (공개+비공개).
 */
export async function getMyPosts(take: number, cursor?: string): Promise<PaginationResponse<PostWithBook>> {
  const userId = await requireUserId()
