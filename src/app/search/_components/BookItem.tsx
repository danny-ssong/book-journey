...
export default function BookItem({ book }: { book: Book }) {
  return (
    <article className="mb-6 flex gap-4 overflow-hidden py-2">
      <BookThumbnail title={book.title} thumbnailUrl={book.thumbnailUrl} className="sm:h-[174px] sm:w-[120px]" />
      <div className="flex flex-1 flex-col gap-2">
        <BookTitle title={book.title} isbn={book.isbn} asLink className="font-semibold" />
        <AuthorName authorName={book.author.name} asLink className="text-black" />
