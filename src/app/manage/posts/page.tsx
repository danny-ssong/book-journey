  ...
  return (
    <section aria-labelledby={headerId}>
      <header className="mb-5 flex items-center justify-between px-4">
        ...
        <ViewToggleButtons isExpanded={isExpanded} setIsExpanded={setIsExpanded} />
      </header>
      <InfiniteScroll fetchNextPage={fetchNextPage} hasNextPage={hasNextPage} isLoading={isLoading}>
        <ul className="min-h-[500px]">
          {posts.map((post) => (
            <li key={post.id}>{isExpanded ? <MyPostCardExpanded post={post} /> : <MyPostCardCompact post={post} />}</li>
          ))}
        </ul>
      </InfiniteScroll>
    </section>
