...
import { createPost, deletePost, getPost, getUserPosts, updatePost } from '@/api/client/post'
...
export const postKeys = {
  ...
  infiniteUser: (userId: string) => [...postKeys.infinite(), 'user', userId] as const,
}
...
export function useUpdatePost() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, updatePostData }: { id: number; updatePostData: UpdatePost }) => updatePost(id, updatePostData),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({
        queryKey: postKeys.detail(id.toString()),
      })
      queryClient.invalidateQueries({ queryKey: postKeys.lists() })
      queryClient.invalidateQueries({ queryKey: postKeys.infinite() })
    },
